...
export const vendorsignup = async (req, res, next) => {
  const { name, email, password, avatar, carNumber,carModel,departure, destination} = req.body;

  const hasedPassword = bcryptjs.hashSync(password, 10);

  const newUser = new Vendor({ name, email, password: hasedPassword, avatar, carNumber,carModel,departure, destination });
